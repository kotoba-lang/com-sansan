(ns sansan.main
  "Kotodama WASM entrypoint for the Sansan clean-room actor (L4) — Clojure port.
  ...
  facts (`sansan.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "sansan")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "sansan_acc"
    :fields [:name :domain :industry :ownerId :annualRevenue] :required [:name :domain]
    :coerce {:annualRevenue :float}      :refs {}}
   {:entity "Contact"     :plural "contacts"     :id-prefix "sansan_con"
    :fields [:accountId :firstName :lastName :email :phone] :required [:firstName :lastName]
    :coerce {}                       :refs {:accountId "Account"}}
   {:entity "Lead"        :plural "leads"        :id-prefix "sansan_lea"
    :fields [:company :email :status :source :score] :required [:company :email]
    :coerce {:score :int}                :refs {}}
   {:entity "Opportunity" :plural "opportunities" :id-prefix "sansan_opp"
    :fields [:accountId :name :stage :amount :closeDate] :required [:name :stage]
    :coerce {:amount :float}         :refs {:accountId "Account"}}
   {:entity "Activity"    :plural "activities"    :id-prefix "sansan_act"
    :fields [:subjectId :type :subject :dueDate :done] :required [:type :subject]
    :coerce {:done :bool}            :refs {}}
   {:entity "Pipeline"    :plural "pipelines"    :id-prefix "sansan_pip"
    :fields [:name :stageOrder :ownerId] :required [:name :stageOrder]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"sansan.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "sansan-compat" :tier tier :entities entities} 200])
...
(defn start! [] :sansan-compat/ready)
